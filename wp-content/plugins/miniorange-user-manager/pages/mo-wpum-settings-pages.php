		...
		<div class="mo_wpum_table_layout">
			...
			<form id="mo-wpum-settings-enable" name="mo-wpum-settings-enable" method="post" action="">
				<input type="hidden" name="option" value="mo_wpum_save_enable" />
					...
					<div>
						<br><br>
						<input type="checkbox" id="send_email_enable" name="mo_wpum_send_email_enable" <?php if(!$mo_manager_utility->is_registered()) echo 'disabled'?> value="1" <?php checked( get_option('mo_wpum_send_email_enable') == 1 );?> />
						<strong>Enable Email Notifications</strong><br>
						<span style="font-style:italic;margin-left:25px;display:block">A notification email will be send to 
							<b><?php echo isset($customer_email) ? $customer_email : 'admin'; ?></b>
							 when a user tries to register and when registration is complete.
						</span><br>								
					</div>
					<div>
					
								<input type="checkbox" id="admin_dashboard_enable" name="mo_wpum_admin_dashboard_enable" <?php if(!$mo_manager_utility->is_registered()) echo 'disabled'?> value="1" <?php checked( get_option('mo_wpum_admin_dashboard_enable') == 1 );?> /><strong>Allow non-admin users to see admin panel?</strong><br><br>
					</div><br>
					<div>
