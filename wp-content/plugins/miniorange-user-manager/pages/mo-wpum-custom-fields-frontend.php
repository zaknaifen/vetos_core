				...
				if( $field->title != NULL){
					if(get_option('field'.$i) == 1){
						if(($field->field_type == 'radio') || ($field->field_type == 'checkbox')){
							...
							for($l=0; $l<$count1; $l++){
								?> 		
									<input type="<?php echo $field->field_type ?>" name="field<?php echo $i ?>" id="field<?php echo $i ?>" class="input" value="" style="<?php if(($field->field_type == 'checkbox') ||($field->field_type == 'radio')){ ?> width:15px;height:16px;<?php } ?>"  /><?php	echo $list[$l]; ?> <br>
								<?php 
							}
						}else{
							?>		
								<label for="field<?php echo $i ?>"><?php echo $field->title ?><br />
