<?php
	function mo_wpum_show_user_signups_page(){
		...
		if( isset( $_GET['action'] ) && $_GET['action'] == "activate" ){
			mo_wpum_show_activate_confirmation_page($_GET['signup_id']);
		}
		elseif( isset( $_GET['action'] ) && $_GET['action'] == "delete" ){
			mo_wpum_show_delete_confirmation_page($_GET['signup_id']);
		}
		elseif( isset( $_GET['action'] ) && $_GET['action'] == "resend" ){
			mo_wpum_show_email_activation_confirm_page($_GET['signup_id']);
		}
		?>
		<div class="wrap">
			<h1>
	...
	function mo_wpum_show_activate_confirmation_page($signup_id){ 
	...
	function mo_wpum_show_delete_confirmation_page($signup_id){
		global $wpdb, $db_queries;
	...
	function mo_wpum_show_email_activation_confirm_page($signup_id){
		global $wpdb, $db_queries;
		$url = admin_url().'users.php?page=mo_wpum_user_signups';
		$signup_user_detail = $db_queries->get_customer_signup_detail( $signup_id );
		$user_id = $wpdb->get_var( $wpdb->prepare( "SELECT ID FROM {$wpdb->users} WHERE user_login LIKE %s", $wpdb->esc_like($signup_user_detail->user_login) ) );
